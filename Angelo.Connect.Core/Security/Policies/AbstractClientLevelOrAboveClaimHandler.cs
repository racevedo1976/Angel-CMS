...
{
    public abstract class AbstractClientLevelOrAboveClaimHandler<TRequirement> : AuthorizationHandler<TRequirement>
    where TRequirement : IAuthorizationRequirement
    {
        ...
        public AbstractClientLevelOrAboveClaimHandler(IContextAccessor<AdminContext> adminContextAccessor)
        {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, TRequirement requirement)
        {
            ...
            if (ValidClaimTypes != null && ValidClaimTypes.Count() > 0)
            {
                ...
                var client = adminContext.ClientContext?.Client;

                if (client == null)
                    throw new UnauthorizedAccessException("Policy failure because client context is invalid");
