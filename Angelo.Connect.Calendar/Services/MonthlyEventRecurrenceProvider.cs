...
{
    public class MonthlyEventRecurrenceProvider : IRecurringEvent
    {
        ...
        public DateTime CalculateEndDate()
        {
            ...
            else if (EventDetail.RecurrenceDetails.Count != null)
            {
                ...
                endDate = EventDetail.EventStart.AddMonths(EventDetail.RecurrenceDetails.Count.Value * EventDetail.RecurrenceDetails.Interval);
                ...
                while (!dayMatchFound)
                {
                    ...
                    if (endDate.Day == EventDetail.RecurrenceDetails.DayOfMonth)
                    {
                        dayMatchFound = true;
                    }
                }
        ...
        private IEnumerable<CalendarEvent> GenerateRecurringEvents(DateTime resolvedRecurringStartDate, DateTime? resolvedRecurringEndDate)
        {
            ...
            var monthNumber = resolvedRecurringStartDate.Month;
            var dayOfMonth = EventDetail.RecurrenceDetails.DayOfMonth;
            var thisEventYear = resolvedRecurringStartDate.Year;

            var newEventDate = DateTime.Parse($"{monthNumber.ToString()}/{dayOfMonth.ToString()}/{thisEventYear.ToString()} {resolvedRecurringStartDate.ToString("h:mm tt")}" );
            ...
            while (newEventDate <= resolvedRecurringEndDate)
            {
                ...
                newEventDate = newEventDate.AddMonths(EventDetail.RecurrenceDetails.Interval);
            }
            return newEvents;
        }
