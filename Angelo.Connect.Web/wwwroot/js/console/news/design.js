define(["console/tools", "resources/news"], function (tools, resx) {
    return {
        attach: function ($self) {
            ...
            var $form = $self.find("#newsPostSummary form");
            ...
            var newsPostId = $form.find("input[name=Id]").val();
                ...
                function performUpdate() {
                    ...
                    $form.form().post()
                        .done(function (model) {
                            ...
                            tools.navigate("/sys/console/news/posts/" + model.Id)
                                .done(function () {
            ...
            $toolbar.find("a[name=delete]").on("click", function () {              
                tools.confirm(resx.newsPostConfirmDelete, { notes: resx.cantUndo }).done(function () {
                    $.ajax({
                        url: "/sys/api/news/posts/" + newsPostId + "?version=" + versionCode,
                        type: "DELETE"
                    })
                    .done(function () {
                        tools.message(resx.newsPostDeleted, { icon: "fa fa-trash-o", sticky: true, hideContent: true });
                    });
                });
            });
            ...
            var $categoryList = $self.find("ul[name=newsPostCategoryList]")
            ...
            function toggleCategoryAlert() {
                var $alert = $self.find("div[name=newsPostCategoryAlert]");
