define(["console/tools", "resources/blog"], function (tools, resx) {
    return {
        attach: function ($self) {
            ...
            var $form = $self.find("#blogPostSummary form");
            ...
            var blogPostId = $form.find("input[name=Id]").val();
                ...
                function performUpdate() {
                    ...
                    $form.form().post()
                        .done(function (model) {
                            ...
                            tools.navigate("/sys/console/blog/posts/" + model.Id)
                                .done(function () {
            ...
            $toolbar.find("a[name=delete]").on("click", function () {              
                tools.confirm(resx.blogPostConfirmDelete, { notes: resx.cantUndo }).done(function () {
                    $.ajax({
                        url: "/sys/api/blog/posts/" + blogPostId + "?version=" + versionCode,
                        type: "DELETE"
                    })
                    .done(function () {
                        tools.message(resx.blogPostDeleted, { icon: "fa fa-trash-o", sticky: true, hideContent: true });
                    });
                });
            });
            ...
            var $categoryList = $self.find("ul[name=blogPostCategoryList]")
            ...
            function toggleCategoryAlert() {
                var $alert = $self.find("div[name=blogPostCategoryAlert]");
